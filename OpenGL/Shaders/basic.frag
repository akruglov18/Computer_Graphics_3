...
SCamera initializeDefaultCamera()
{
	//** CAMERA **//
	SCamera camera;
	camera.Position = vec3(0.0, 0.0, -7.9);
...
void initializeDefaultScene(out STriangle triangles[12], out SSphere spheres[2])
{
	...
	triangles[0].v2 = vec3(-5.0, 5.0, 8.0);
	triangles[0].v3 = vec3(-5.0, 5.0,-8.0);
	triangles[0].MaterialIdx = 0;

	triangles[1].v1 = vec3(-5.0,-5.0,-8.0);
	triangles[1].v2 = vec3(-5.0,-5.0, 8.0);
	triangles[1].v3 = vec3(-5.0, 5.0, 8.0);
	triangles[1].MaterialIdx = 0;

	/* back wall */
	triangles[2].v1 = vec3(-5.0,-5.0, 8.0);
	triangles[2].v2 = vec3( 5.0,-5.0, 8.0);
	triangles[2].v3 = vec3(-5.0, 5.0, 8.0);
	triangles[2].MaterialIdx = 4;
	triangles[3].v1 = vec3( 5.0, 5.0, 8.0);
	triangles[3].v2 = vec3(-5.0, 5.0, 8.0);
	triangles[3].v3 = vec3( 5.0,-5.0, 8.0);
	...
	triangles[7].v2 = vec3( 5.0, 5.0, 8.0);
...
bool Raytrace ( SRay ray, SSphere spheres[2], STriangle triangles[12], SMaterial materials[6], float start, float final, inout SIntersection intersect )
{
	...
	if(intersect.Color[0]==0 && intersect.Color[1]==0 && intersect.Color[2]==0)
		intersect.Color==vec3(1.0,1.0,1.0);
	return result;
}
...
struct STracingRay
{
	...
	STracingRay stray[10000];
	int k=0;
	void pushRay(STracingRay newray){
		if(k<10001){
			stray[k]=newray;
			k++;
		}
...
void main ( void )
{
	...
	//{
	//	resultColor = Phong(intersect,light,Shadow(light, intersect),uCamera);
	//}
